...
import com.leaf.hopperfill.geometry.LineBresenham3D;
...
public class FillValidator {
    ...
    public static Result validate(ServerPlayerEntity player, ServerWorld world,
        ...
        for (BlockPos hopperPos : hoppers) {
            if (!(world.getBlockState(hopperPos).getBlock() instanceof HopperBlock)) {
                return Result.fail("§c错误：" + hopperPos.toShortString() + " 不是漏斗");
            }
        }

        List<ItemStack> results = RegionScanner.scan(world, from, to, skipIds, lineStart, lineEnd);
        ...
        for (ItemStack r : results) {
            ...
            if (r.getMaxCount() == 1) {
                return Result.fail("§c错误：扫描到不可堆叠物品 \"" + r.getName().getString() + "\"");
            }
        }
        ...
        for (ItemStack r : results) {
            ...
            if (!TemplateStorage.hasAnyItem(needed)) {
                ...
                return Result.fail("§c错误：扫描到 " + typeName + " 堆叠物品 \"" + r.getName().getString() +
